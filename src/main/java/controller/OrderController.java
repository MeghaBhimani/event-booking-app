...
public class OrderController {
    ...
    public OrderController(Stage stage, Model model) {
        this.stage = stage;
        this.model = model;
    }
    ...
    public void loadOrders() {
        ...
        try {
            List<Order> orderList = orderDao.getOrdersForUser(model.getCurrentUserId());
            orders = FXCollections.observableArrayList(orderList);
            orderTableView.setItems(orders);
        } catch (SQLException ex) {
            System.err.println("Failed to load orders: " + ex.getMessage());
        }
    }
    ...
    private void exportOrdersToFile() {
        ...
        chooser.setInitialFileName("my_orders.txt");
